...
- `Accent` は `〔…〕` の中のアクセント分解表記を 1 文字ずつ解決したもの。`〔e'〕` なら
  `code` は面区点 `1-09-63`、`name` は文字名 `アキュートアクセント付きE小文字`、
...
- `Break` の `None` という名前。JSON では文字列 `"None"` になり、`null` と紛らわしい。
  /2 での改名を検討。
- 左ルビ。`RubyDirection` に `Left` を定義してあるが、左ルビの記法（`「…」の左に
  「…」のルビ`）を現行の生産者は解釈せず、生成時から編集者注（`Note`）になる。
  元の変換系は解決して左側に付けるので、ここは互換の穴でもある。`Left` を持つ木が
  生まれるのは解決を実装してから。
