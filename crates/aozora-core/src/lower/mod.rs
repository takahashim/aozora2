...
pub enum LowerDiagnosticKind {
    ...
    /// 行の途中で複数行ブロックを開いた（マーカーの前に空白でない内容がある）。
    ///
    /// 参照は変換を中止しない（字下げだけは nil を踏んでクラッシュする）ので Warning に
    /// 留める。ただし注記一覧に例が無く（全例・記入例とも `ここから…` は 46 箇所すべて
    /// 行頭。実測）、開始タグが `<div>` である以上**必ず行が割れる**ので、書いた見た目
    /// どおりには出ない。
    ///
    /// 開始タグの直後から本文が始まる形（マーカーの前が空か空白だけ）は実文書にあり
    /// （001065/18361・001841/57318）、行も割れないので対象にしない。
    MidlineBlockOpen(BlockKind),
}
