...
pub enum InlineKind {
    ...
    ///
    /// **記法ではない**。青空文庫記法に「ここで改行」に当たる書き方は無いので、効果ではなく
    /// 原因を名前にしてある。ただし多くは誤記でもなく、日本語の亀甲括弧 `〔…〕` が行を
    /// またいだだけである（アクセント記法が同じ括弧を使うので、行内で閉じない亀甲括弧が
    /// 未閉じアクセントに見える）。コーパス全 53 箇所のうち 47 箇所は後の行の `〕` で
    /// 閉じている。
    ///
